    ...
    <b-row>
      <b-col md="3" sm="6" class="mb-3">
        <b-card class="text-center">
          <div class="h5 mb-0">{{ totalCategoriesDisplay }}</div>
          <small class="text-muted">{{ $t('Total_Categories') }}</small>
        </b-card>
      </b-col>
      <b-col md="3" sm="6" class="mb-3">
        <b-card class="text-center">
          <div class="h5 mb-0">{{ syncedCategoriesDisplay }}</div>
          <small class="text-muted">{{ $t('Synced_Categories') }}</small>
        </b-card>
      </b-col>
      <b-col md="3" sm="6" class="mb-3">
        <b-card class="text-center">
...
export default {
  ...
  methods: {
    load() {
      ...
      const p2 = axios.get('categories', { params: { limit: 1 } }).then(({ data }) => { this.totalCategories = data.totalRows != null ? data.totalRows : null; });
      const p3 = axios.get('woocommerce/categories/unsynced-count').then(({ data }) => { this.unsyncedCount = data.count; });
      return Promise.all([p1,p2,p3]);
    },
    manualSync(onlyUnsynced) {
      this.syncing = true;
      let url = 'woocommerce/sync/categories?mode=push';
